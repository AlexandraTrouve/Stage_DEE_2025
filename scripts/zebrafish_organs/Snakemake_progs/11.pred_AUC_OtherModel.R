...
rocPrArea<-function(positive, negative) {
  ...
  for (i in 2:8) {
    pos<-cbind(positive[[i]],positive[,9])
    neg<-cbind(negative[[i]],negative[,9])
...
## T6 prediction based on T1 model
pos6<-fread(pos_files[[6]])
neg6<-fread(neg_files[[6]])
## T7 prediction based on T1 model
pos7<-fread(pos_files[[7]])
neg7<-fread(neg_files[[7]])

pos<-cbind(pos1,pos2$V2,pos3$V2,pos4$V2,pos5$V2,pos6$V2,pos7$V2)
neg<-cbind(neg1,neg2$V2,neg3$V2,neg4$V2,neg5$V2,neg6$V2,neg7$V2)
