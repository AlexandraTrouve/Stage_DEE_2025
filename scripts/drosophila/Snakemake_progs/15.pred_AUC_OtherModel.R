...
rocPrArea<-function(positive, negative) {
  ...
  for (i in 2:6) {
    pos<-cbind(positive[[i]],positive[,7])
    neg<-cbind(negative[[i]],negative[,7])
...
##### take T1 promoters as an example #####
...
## T3 prediction based on T1 model 
pos3<-fread(pos_files[[3]])
neg3<-fread(neg_files[[3]])
## T4 prediction based on T1 model 
pos4<-fread(pos_files[[4]])
neg4<-fread(neg_files[[4]])
## T5 prediction based on T1 model 
pos5<-fread(pos_files[[5]])
neg5<-fread(neg_files[[5]])

pos<-cbind(pos1,pos2$V2,pos3$V2,pos4$V2,pos5$V2)
neg<-cbind(neg1,neg2$V2,neg3$V2,neg4$V2,neg5$V2)
...
plotColors<-c("pink","lightsalmon","lightblue","thistle2","darkseagreen3","snow3")
names(plotColors) <- c("hourglass","inverse_hourglass","early_constraint","late_constraint","uniform_high","uniform_low")
...
legend(0.3,0.35,paste(plotLegend[[1]]," ","(AUC = ",round(aucOthers[[3]][1],digits = 3),")",sep=""),lwd = 3,bty="n",cex=1.4,col = plotColors[plotLegend[[1]]]) 

for (i in 2:5) {
  lines(unlist(aucOthers[[1]][i]),unlist(aucOthers[[2]][i]),col=plotColors[plotLegend[[i]]],lwd=3)}

legendY<-c(0.3, 0.25, 0.2, 0.15, 0.1)
for (i in 2:5) {
